...
大学生不运动的本质，不是懒，是**大学生活的默认结构里没有运动的位置**。绝大多数 fitness 产品的解法是给运动加奖励（卡路里、徽章、连续天数）—— 这条路打不过短视频，steam和网游。

同行的解法不是奖励个体，是**把运动嵌进有归属感的团队结构里**。篮球队员"坚持运动"的原因——除了热爱，还有他属于一支队伍，到场是责任和荣誉。

产品的角色是**守护者**——它不组织、不激励，做三件事：

1. 让一次相聚运动**容易发生**
2. 让相聚被**沉淀得可感**（不是数字，是可回忆起的事件）
3. 让Agent和互联网**为这个团队**不断创造再聚的诱因，创造运动场景和条件
...
- **/team/:id 顶部 诱因卡** —— 任何人可点击展开，看「它」的 reasoning（按"团队历史 / 当下环境 / 实时场地 / 其他团队此刻"四类信号）；成员可点「也到」响应

**静态展示**（产品论证的可视化）：

- **/team/:id 「美团 · 今晚的准备」条** —— 仅成员可见，trigger 存在时出现。3 行静态 mock（球鞋包/水/场地 已通过美团柜或外卖到位），把 PRD 第十六节"美团生态给条件"在产品里显形。按 team.field 选三种 mock 内容（球类晚场 / 晨跑 / 骑行）
- **首屏弹窗** —— 每次刷新弹一次，30 秒读完产品立场（Agent 给理由 + 美团生态给条件 + 团队带来的三件事 + "系统性增加运动频率"收尾）
...
│   │   │   ├── _TriggerCard.tsx  # 客户端：诱因卡可展开（含 Agent 4 类信号 reasoning）+ 响应
│   │   │   ├── _PrepStrip.tsx    # 美团生态给条件 的静态演示（仅成员可见）
│   │   │   ├── _ActionBar.tsx    # 客户端:底部行动条（仅成员可见）+ 弹窗
│   │   │   ├── _NewLifeChangeButton.tsx  # 客户端：发动态 sheet（仅成员可见）
│   │   │   └── end/page.tsx   # 屏 4 · 尽头
